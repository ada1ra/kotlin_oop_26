...
open class SingleLinkedList(initSize: Int = 2) : CustomList {
    ...
    override fun add(element: Int) {
        if (head == null) head = Node(element)
        else {
            var current = head
    ...
    companion object {
        fun singleLinkedListOf(vararg items: Int) = items.fold(SingleLinkedList(items.size)) { list, item ->
